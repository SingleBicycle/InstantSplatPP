...
def train(args):
    ...
    #new code
    train_tools.init_distributed_mode(args)
    ...
    # ------------------------------
    # Datasets and Dataloaders (schema-agnostic)
    # Supports either:
    #   A) args.dataset.train_dataset / test_dataset / num_workers
    #   B) args.dataset.dataset_str (train only), optional num_workers
    # ------------------------------
    def _get(cfg, key, default=None):
        return getattr(cfg, key) if hasattr(cfg, key) else default

    # Train spec
    train_spec = _get(args.dataset, "train_dataset", None)
    if train_spec is None:
        train_spec = _get(args.dataset, "dataset_str", None)
    if train_spec is None:
        raise RuntimeError(
            "Dataset spec not found: expected dataset.train_dataset or dataset.dataset_str"
        )

    # num_workers (fallback to a sane default)
    num_workers = _get(args.dataset, "num_workers", None)
    if num_workers is None:
        if num_workers is None:
            cpu_count = os.cpu_count() or 4
            num_workers = max(2, cpu_count // 2)

    print(f"Building train dataset: {train_spec}")
    data_loader_train = build_dataset(
        dataset=train_spec,
    ...
    # Test spec (optional)
    test_spec = _get(args.dataset, "test_dataset", None)
    data_loader_test = {}
    if test_spec:
        # num_views may still be an interpolated string; guard conversion.
        try:
            n_views_cfg = int(args.dataset.num_views)
        except Exception:
            n_views_cfg = 2
        test_batch_size = 2 * (
            args.train_params.max_num_of_imgs_per_gpu // max(1, n_views_cfg)
        )
        print(f"Building test datasets from: {test_spec}")
        data_loader_test = {
            ds.split("(")[0]: build_dataset(
                dataset=ds,
                num_workers=num_workers,
                test=True,
                batch_size=test_batch_size,
            )
            for ds in test_spec.split("+")
            if "(" in ds
        }
    ...
    use_ddp = torch.distributed.is_initialized() and getattr(args, "distributed", False)
...
@hydra.main(version_base=None, config_path=None)  # allow CLI --config-path/--config-name
def main(cfg: DictConfig):
    print(">>> entering train() with Hydra cfg", flush=True)  # quick sanity print
    train(cfg)  # call your existing train() function
