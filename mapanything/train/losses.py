...
def _project_points_to_image_grid(
    ...
    """Project world-space points onto image grid and return normalized sampling grid and validity mask."""

    R = camera_pose[:, :3, :3]
    t = camera_pose[:, :3, 3]
    pts_rel = points_world - t.unsqueeze(1)
    world_to_cam = R.transpose(-1, -2)
    cam_points = torch.einsum("bij,bnj->bni", world_to_cam, pts_rel)
    ...
    valid = valid & (u >= 0) & (u <= (width - 1)) & (v >= 0) & (v <= (height - 1))

    denom_x = max(width - 1, 1)
    denom_y = max(height - 1, 1)
    x_norm = (2.0 * u / denom_x) - 1.0
    y_norm = (2.0 * v / denom_y) - 1.0
    grid = torch.stack((x_norm, y_norm), dim=-1)

    return grid, valid


def _sample_map_at_grid(feature_map: torch.Tensor, grid: torch.Tensor) -> torch.Tensor:
    """Sample 2D feature map at provided grid positions using bilinear interpolation."""

    feature_map = feature_map.unsqueeze(1)  # (B, 1, H, W)
...
class KeepPointSelectionLoss(MultiLoss):
    """Noisy-OR point selection loss using decoder keep logits and uncertainty."""

    def __init__(
        self,
        anchor_view_idx: int = 0,
        knn_k: int = 8,
        max_points: int = 8192,
        lambda_rep: float = 0.2,
        lambda_card: float = 0.1,
        cardinality_target: float = 0.3,
        min_precision: float = 1.0,
        conf_floor: float = 1e-4,
    ) -> None:
        super().__init__()
        self.anchor_view_idx = anchor_view_idx
        self.knn_k = knn_k
        self.max_points = max_points
        self.lambda_rep = lambda_rep
        self.lambda_card = lambda_card
        self.cardinality_target = cardinality_target
        self.min_precision = min_precision
        self.conf_floor = conf_floor

    def get_name(self):
        return "KeepPointSelectionLoss"

    def _get_log_variance_map(self, pred_view, device):
        log_var = pred_view.get("log_variance")
        if log_var is not None:
            return log_var.to(device)
        conf_map = pred_view.get("conf")
        if conf_map is not None:
            conf_map = conf_map.to(device)
            return -torch.log(conf_map.clamp(min=self.conf_floor))
        keep_logits = pred_view["keep_logits"].to(device)
        return torch.zeros_like(keep_logits)

    def compute_loss(self, batch, preds, **kw):
        if not preds:
            raise ValueError("Empty predictions passed to KeepPointSelectionLoss")
        if "keep_logits" not in preds[self.anchor_view_idx]:
            raise ValueError("keep_logits missing from model outputs; enable decoder keep head")

        num_views = len(preds)
        pred_anchor = preds[self.anchor_view_idx]["pts3d"]
        device = pred_anchor.device
        batch_size, height, width, _ = pred_anchor.shape
        num_points = height * width

        pred_points_flat = pred_anchor.view(batch_size, num_points, 3)
        pred_points_detached = pred_points_flat.detach()
        gt_points_flat = batch[self.anchor_view_idx]["pts3d"].to(device).view(batch_size, num_points, 3)
        residual = torch.norm(pred_points_detached - gt_points_flat, dim=-1)

        valid_mask = batch[self.anchor_view_idx]["valid_mask"].to(device).view(batch_size, num_points).bool()

        prob_samples_list = []
        log_var_samples_list = []
        mask_list = []

        for view_idx in range(num_views):
            keep_logits_map = preds[view_idx].get("keep_logits")
            if keep_logits_map is None:
                raise ValueError(f"keep_logits missing for view {view_idx}")
            keep_logits_map = keep_logits_map.to(device)
            log_var_map = self._get_log_variance_map(preds[view_idx], device)

            if view_idx == self.anchor_view_idx:
                probs = torch.sigmoid(keep_logits_map.view(batch_size, -1))
                log_vars = log_var_map.view(batch_size, -1)
                mask_view = valid_mask
            else:
                camera_pose = batch[view_idx].get("camera_pose")
                if camera_pose is None:
                    camera_pose = batch[view_idx].get("camera_poses")
                if camera_pose is None:
                    raise ValueError(f"camera pose missing for view {view_idx}")
                camera_pose = camera_pose.to(device)

                camera_intrinsics = batch[view_idx].get("camera_intrinsics")
                if camera_intrinsics is None:
                    camera_intrinsics = batch[view_idx].get("intrinsics")
                if camera_intrinsics is None:
                    raise ValueError(f"camera intrinsics missing for view {view_idx}")
                camera_intrinsics = camera_intrinsics.to(device)

                image_shape = (keep_logits_map.shape[-2], keep_logits_map.shape[-1])
                grid, proj_valid = _project_points_to_image_grid(
                    pred_points_detached, camera_pose, camera_intrinsics, image_shape
                )
                sampled_logits = _sample_map_at_grid(keep_logits_map, grid)
                probs = torch.sigmoid(sampled_logits)
                log_vars = _sample_map_at_grid(log_var_map, grid)
                mask_view = valid_mask & proj_valid

            probs = torch.where(mask_view, probs, torch.zeros_like(probs))
            log_vars = torch.where(mask_view, log_vars, torch.zeros_like(log_vars))

            prob_samples_list.append(probs)
            log_var_samples_list.append(log_vars)
            mask_list.append(mask_view)

        prob_stack = torch.stack(prob_samples_list, dim=1)
        mask_stack = torch.stack(mask_list, dim=1)
        log_var_stack = torch.stack(log_var_samples_list, dim=1)

        valid_any = mask_stack.any(dim=1)
        one_minus = torch.where(mask_stack, 1.0 - prob_stack, torch.ones_like(prob_stack))
        q = 1.0 - torch.prod(one_minus, dim=1)
        q = torch.where(valid_any, q, torch.zeros_like(q))
        q = q.clamp(0.0, 1.0)

        log_var_sum = torch.where(mask_stack, log_var_stack, torch.zeros_like(log_var_stack)).sum(dim=1)
        counts = mask_stack.sum(dim=1).clamp(min=1)
        log_var = log_var_sum / counts
        log_var = torch.where(valid_any, log_var, torch.zeros_like(log_var))
        precision = torch.exp(-log_var)
        if self.min_precision is not None:
            precision = torch.clamp(precision, min=self.min_precision)
        log_var = -torch.log(precision)

        residual_sq = residual**2
        unc_map = 0.5 * (precision * residual_sq + log_var)
        combined_valid = valid_mask & valid_any
        valid_float = combined_valid.float()
        if valid_float.sum() > 0:
            loss_unc = (unc_map * valid_float).sum() / valid_float.sum()
        else:
            loss_unc = unc_map.sum() * 0.0

        loss_rep = residual.new_tensor(0.0)
        rep_count = 0
        for b in range(batch_size):
            valid_idx = torch.nonzero(combined_valid[b], as_tuple=False).squeeze(1)
            if valid_idx.numel() <= 1:
                continue
            if valid_idx.numel() > self.max_points:
                perm = torch.randperm(valid_idx.numel(), device=device)[: self.max_points]
                valid_idx = valid_idx[perm]
            pts_sub = pred_points_flat[b, valid_idx].detach()
            q_sub = q[b, valid_idx]
            n_pts = pts_sub.shape[0]
            k = min(self.knn_k, n_pts - 1)
            if k <= 0:
                continue
            dist = torch.cdist(pts_sub.unsqueeze(0), pts_sub.unsqueeze(0)).squeeze(0)
            knn_idx = dist.topk(k + 1, largest=False).indices[:, 1:]
            row_idx = torch.arange(n_pts, device=device).unsqueeze(1).expand(-1, k).reshape(-1)
            col_idx = knn_idx.reshape(-1)
            rep_vals = torch.relu(q_sub[row_idx] + q_sub[col_idx] - 1.0)
            if rep_vals.numel() == 0:
                continue
            loss_rep = loss_rep + rep_vals.mean()
            rep_count += 1
        if rep_count > 0:
            loss_rep = loss_rep / rep_count
        else:
            loss_rep = loss_rep * 0.0

        loss_card = residual.new_tensor(0.0)
        card_count = 0
        for b in range(batch_size):
            valid_idx = torch.nonzero(combined_valid[b], as_tuple=False).squeeze(1)
            if valid_idx.numel() == 0:
                continue
            mean_q = q[b, valid_idx].mean()
            loss_card = loss_card + (mean_q - self.cardinality_target) ** 2
            card_count += 1
        if card_count > 0:
            loss_card = loss_card / card_count
        else:
            loss_card = loss_card * 0.0

        total_loss = loss_unc + self.lambda_rep * loss_rep + self.lambda_card * loss_card

        details = {
            f"{self.get_name()}_L_unc": float(loss_unc.detach()),
            f"{self.get_name()}_L_rep": float(loss_rep.detach()),
            f"{self.get_name()}_L_card": float(loss_card.detach()),
        }
        if combined_valid.any():
            q_mean = (q * valid_float).sum() / valid_float.sum()
            details[f"{self.get_name()}_q_mean"] = float(q_mean.detach())
