...
class ModularDUSt3R(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if keep_head_enabled and self.pred_head_type != "dpt":
            raise ValueError("Keep/uncertainty head requires DPT decoder outputs")
    ...
    def _downstream_head(self, head_num, decout, img_shape):
        "Run the respective prediction heads and expose decoder features."
        if self.pred_head_type == "linear":
            ...
            feature_head = getattr(self, f"dpt_feature_head{head_num}")
            regressor_head = getattr(self, f"dpt_regressor_head{head_num}")
            head_input = PredictionHeadLayeredInput(
                list_features=decout[f"{head_num}"], target_output_shape=img_shape
            )
            feature_output = feature_head(head_input)
            ...
            head_output = regressor_head(feature_output)
        else:
            raise ValueError(f"Unsupported pred_head_type: {self.pred_head_type}")

        return head_output, decoder_feature
    ...
    def forward(self, views):
        ...
        with torch.autocast("cuda", enabled=False):
            # Prediction heads
            head_output1, decoder_feature1 = self._downstream_head(
                1, info_sharing_outputs, shape1
            )
            head_output2, decoder_feature2 = self._downstream_head(
                2, info_sharing_outputs, shape2
            )
            ...
            if self.keep_head is not None:
                keep_logits1, log_var1 = self.keep_head(decoder_feature1)
                keep_logits2, log_var2 = self.keep_head(decoder_feature2)
                keep_logits1 = keep_logits1.squeeze(1)
                keep_logits2 = keep_logits2.squeeze(1)
                res1["keep_logits"] = keep_logits1
                res1["keep_probability"] = torch.sigmoid(keep_logits1)
                if log_var1 is not None:
                    res1["log_variance"] = log_var1.squeeze(1)
                res2["keep_logits"] = keep_logits2
                res2["keep_probability"] = torch.sigmoid(keep_logits2)
                if log_var2 is not None:
                    res2["log_variance"] = log_var2.squeeze(1)
            res = [res1, res2]
