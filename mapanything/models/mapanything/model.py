...
from mapanything.models.mapanything.keep_head import KeepUncertaintyHead
...
class MapAnything(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        self.keep_head_config = pred_head_config.get("keep_head", {})
    ...
    def _initialize_prediction_heads(self, pred_head_config):
        ...
        keep_head_enabled = bool(self.keep_head_config.get("enabled", False))
        if keep_head_enabled and "dpt" not in self.pred_head_type:
            raise ValueError("Keep/uncertainty head requires a DPT-based dense head")

        if keep_head_enabled:
            hidden_dims = self.keep_head_config.get("hidden_dims", (128, 64))
            activation = self.keep_head_config.get("activation", "relu")
            predict_log_variance = self.keep_head_config.get("predict_log_variance", True)
            input_channels = self.keep_head_config.get(
                "input_channels", pred_head_config["feature_head"]["feature_dim"]
            )
            self.keep_head = KeepUncertaintyHead(
                in_channels=input_channels,
                hidden_dims=hidden_dims,
                activation=activation,
                predict_log_variance=predict_log_variance,
            )
    ...
    def downstream_dense_head(
        ...
        if self.pred_head_type == "linear":
            ...
            dpt_input = PredictionHeadLayeredInput(
                list_features=dense_head_inputs,
                target_output_shape=img_shape,
            )
            feature_output = self.dpt_feature_head(dpt_input)
            decoder_feature = feature_output.features_upsampled_8x
            if decoder_feature.shape[-2:] != img_shape:
                decoder_feature = F.interpolate(
                    decoder_feature, size=img_shape, mode="bilinear", align_corners=True
                )
            dense_head_outputs = self.dpt_regressor_head(feature_output)
            dense_final_outputs = self.dense_adaptor(
                AdaptorInput(
                    adaptor_feature=dense_head_outputs.decoded_channels,
                    output_shape_hw=img_shape,
        ...
        return dense_final_outputs, decoder_feature
    ...
    def downstream_head(
        ...
        if memory_efficient_inference:
            ...
            for batch_idx in range(num_batches):
                ...
                dense_final_outputs_batch, decoder_features_batch = self.downstream_dense_head(
                    dense_head_inputs_batch, img_shape
                )
                dense_final_outputs_list.append(dense_final_outputs_batch)
                if decoder_features_batch is not None:
                    decoder_features_list.append(decoder_features_batch)
            ...
            if decoder_features_list:
                decoder_features = torch.cat(decoder_features_list, dim=0)
            ...
            # Dense prediction
            dense_final_outputs, decoder_features = self.downstream_dense_head(
        ...
        return dense_final_outputs, pose_final_outputs, scale_final_output, decoder_features
    ...
    def forward(self, views, memory_efficient_inference=False):
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            if self.keep_head is not None and decoder_features is not None:
                keep_logits, log_variance = self.keep_head(decoder_features)
                keep_logits = keep_logits.squeeze(1)
                keep_probs = torch.sigmoid(keep_logits)
                keep_logits_per_view = keep_logits.chunk(num_views, dim=0)
                keep_probs_per_view = keep_probs.chunk(num_views, dim=0)
                log_variance_per_view = (
                    log_variance.squeeze(1).chunk(num_views, dim=0)
                    if log_variance is not None
                    else None
                )
                for i in range(num_views):
                    res[i]["keep_logits"] = keep_logits_per_view[i]
                    res[i]["keep_probability"] = keep_probs_per_view[i]
                    if log_variance_per_view is not None:
                        res[i]["log_variance"] = log_variance_per_view[i]
